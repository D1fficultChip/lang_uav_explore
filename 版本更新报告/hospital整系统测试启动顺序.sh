...
# 如需换任务文件，改这里即可
PLAN_PATH=${PLAN_PATH:-/shared/plan_hospital_group_photo_elevator_task.json}
RUNTIME_CONFIG=${RUNTIME_CONFIG:-/home/young/uav_demo/central_runtime_v0/config_hospital_orchestrated.yaml}
...
docker exec falcon_noetic bash -lc '
  pkill -9 -f "roslaunch perception_bridge hospital_falcon.launch" || true
  pkill -9 -f "roslaunch perception_bridge hospital_falcon_only.launch" || true
  pkill -9 -f "roslaunch perception_bridge hospital_uav_base.launch" || true
  pkill -9 -f "roslaunch perception_bridge hospital_perception.launch" || true
...
echo "[3/7] 启动 hospital Gazebo + UAV 基础环境（不直接启动 FALCON）"
docker exec falcon_noetic bash -lc '
  source /opt/ros/noetic/setup.bash
  source /root/catkin_ws/devel/setup.bash
  nohup xvfb-run -a -s "-screen 0 1280x1024x24" \
    roslaunch perception_bridge hospital_uav_base.launch \
    gui:='"${GAZEBO_GUI}"' \
    init_x:=0.0 init_y:=12.0 init_z:=1.2 init_yaw:=1.57079632679 \
    > /shared/hospital_uav_base.log 2>&1 &
'
sleep 10
...
echo "[4/7] 启动 hospital perception bridge + cue bias + cue heatmap"
docker exec falcon_noetic bash -lc '
  source /opt/ros/noetic/setup.bash
  source /root/catkin_ws/devel/setup.bash
  nohup roslaunch perception_bridge hospital_perception.launch \
