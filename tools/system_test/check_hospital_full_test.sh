...
echo "[CHECK] 关键共享文件"
for f in frame.jpg infer.json infer_cue.json infer_mask.png target_localization.json cue_hist_status.json plan_runtime.jsonl runtime_events.jsonl runtime_status.json runtime_noetic.log hospital_uav_base.log falcon_search.log perception_bridge.log cue_bias.log cue_hist_exporter.log gsam2.log; do
  p="${SHARED}/${f}"
  if [ -f "${p}" ]; then
...
echo "[CHECK] 关键日志尾部"
for f in runtime_noetic.log hospital_uav_base.log falcon_search.log perception_bridge.log cue_bias.log cue_hist_exporter.log gsam2.log; do
  p="${SHARED}/${f}"
  if [ -f "${p}" ]; then
    echo "--- ${f} ---"
    tail -n 20 "${p}" || true
