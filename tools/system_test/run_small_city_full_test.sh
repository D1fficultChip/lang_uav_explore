...
SHARED=${ROOT}/shared
START_SCRIPT="${ROOT}/版本更新报告/small_city整系统测试启动顺序.sh"
...
echo "[INFO] 一键整系统测试启动（small_city）"
echo "  ROOT=${ROOT}"
echo "  SHARED=${SHARED}"
...
echo
echo "[INFO] 启动完成，建议查看："
echo "  - 浏览器: http://localhost:8001/runtime_status.html"
echo "  - 共享目录: ${SHARED}"
echo "  - 状态检查: bash ${ROOT}/tools/system_test/check_small_city_full_test.sh"
