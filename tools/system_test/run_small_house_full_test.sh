...
SHARED=${ROOT}/shared
START_SCRIPT="${ROOT}/版本更新报告/small_house整系统测试启动顺序.sh"
...
export PLAN_PATH="${PLAN_PATH:-/shared/plan_small_house_long_task.json}"
export RUNTIME_CONFIG="${RUNTIME_CONFIG:-/home/young/uav_demo/central_runtime_v0/config_small_house_orchestrated.yaml}"
...
# 补充 HTTP 静态文件服务（仿真脚本漏了这步）
nohup bash -lc "
  cd ${SHARED}
  python3 -m http.server 8001
" > /tmp/http_8001.log 2>&1 &
...
echo "  - 共享目录: ${SHARED}"
echo "  - 状态检查: bash ${ROOT}/tools/system_test/check_small_house_full_test.sh"
