...
def _parse_target(container: str) -> Tuple[str, Optional[str], Optional[int], Optional[str]]:
    """Parse container spec.

    Supported formats:
      - docker: <container_name> (default)
      - ssh: ssh:nv@192.168.1.122
      - ssh: ssh:nv@192.168.1.122:22
      - ssh: ssh://nv@192.168.1.122:22
      - ssh: ssh:192.168.1.122 (uses current user)
      - ssh with identity: ssh:nv@192.168.1.122:22?i=/path/to/key

    Returns: (mode, target, port, identity_file)
      mode: 'docker' or 'ssh'
      target: for docker => container name; for ssh => 'user@host' or 'host'
    """
...
class DockerRoslaunchAdapter(Adapter):
    ...
    def _ssh(self, args: List[str]) -> None:
        ...
        base.append(self.target)  # user@host or host
        base.extend(args)
        subprocess.run(base, check=False)
