...
class RecoveryManager:
    ...
    def suggest(
        ...
        *,
        ...
        if diagnostic_code == DiagnosticCode.SEARCH_STUCK:
            if self.allow_escape and stage_intent == "SEARCH":
                return RecoveryAction(kind="escape", reason="search stuck", stage_id=stage_id)
            if retry_count < self.max_retries_per_stage:
                return RecoveryAction(kind="retry_same_stage", reason="search stuck", stage_id=stage_id)
            if self.allow_fallback_to_search:
                return RecoveryAction(kind="fallback_to_search", reason="search stuck", stage_id=stage_id)
            return RecoveryAction(kind="safe_terminate", reason="search stuck", stage_id=stage_id)
