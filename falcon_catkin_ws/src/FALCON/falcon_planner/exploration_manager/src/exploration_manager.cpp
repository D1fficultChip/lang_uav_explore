...
void ExplorationManager::initialize(ros::NodeHandle &nh) {
  ...
  nh.param("lang/enable", lang_enable_, true);
  nh.param("lang/beta", lang_beta_, 10.0);
  nh.param("lang/timeout", lang_timeout_, 0.5);
  nh.param("lang/conf_min", lang_conf_min_, 0.0);
...
int ExplorationManager::planExploreMotionHGrid(const Vector3d &pos, const Vector3d &vel,
  ...
  if (frontier_ids.size() > 0) {
    ...
    if (lang_enable_ && lang_beta_ > 1e-9 && !frontier_ids_from_sop_path.empty()) {
      ...
      for (int j = 0; j < K; ++j) {
        const int fid = frontier_ids_from_sop_path[j];
        const Eigen::Vector3d& p = ed_->points_[fid];

        // baseline cost（用真实 computeCost；K 很小不会慢）
        std::vector<Eigen::Vector3d> tmp_path;
        double cost = PathCostEvaluator::computeCost(
            pos, p, yaw[0], ed_->yaws_[fid],
            vel, yaw[1], tmp_path);

        if (!std::isfinite(cost) || cost > 499.0) cost = 1e6;  // 不可达就别选
      ...
      // ===== 做法B：一次性打印 topK 列表（同一轮一个 seq）=====
      static int seq = 0;
      ++seq;

      if (lang_debug_) {
        // 拷贝一份用于排序打印（不影响原始 SOP 顺序）
        auto sorted = dbg;
        std::sort(sorted.begin(), sorted.end(),
                  [](const CandDbg& a, const CandDbg& b) { return a.score > b.score; });  // score 越大越好
        ...
        ss << "  (sorted by score desc):\n";
        for (int r = 0; r < (int)sorted.size(); ++r) {
          const auto& c = sorted[r];
          const bool is_pick = (c.j == best_j);  // best_j 是 pick 在原 topK 的位置

          ss << "    r=" << r
            << (is_pick ? "  [PICK] " : "         ")
            << "fid=" << c.fid
            << " (j=" << c.j << ") "
            << "cost=" << std::setprecision(2) << c.cost << " "
            << "h=" << std::setprecision(3) << c.h << " "
            << "score=" << std::setprecision(2) << c.score
            << "\n";
        }
      ...
      if (best_j != 0) {
        std::swap(frontier_ids_from_sop_path[0], frontier_ids_from_sop_path[best_j]);
      }

      // 单行总结（你原来那条保留，并带 seq 方便对齐）
      ROS_WARN_THROTTLE(1.0,
        "[LangBias-MAIN] seq=%d topK=%d pick fid=%d (j=%d) cost=%.2f h=%.3f score=%.2f",
        seq, K, best_fid, best_j, best_cost, best_h, best_score);
    }
  ...
  if (ed_->grid_tour2_.size() <= 1) {
    ...
    for (int i = 0; i < ed_->averages_.size(); i++) {
      if (hierarchical_grid_->getLayerCellId(0, ed_->averages_[i]) == next_cell_id_grid_tour2) {
        ...
        // ===== Lang bias (方案3)：语言奖励降低 cost，让偏好方向更容易被选中 =====
        double h = 0.0;
        ...
        if (lang_debug_) {
          ROS_INFO(
              "[ExplorationManager][LangBias-A] frontier=%d cost=%.2lf h=%.2lf cost'=%.2lf "
              "vp=(%.2lf, %.2lf, %.2lf)",
              i, cost, h, cost_biased, ed_->points_[i].x(), ed_->points_[i].y(), ed_->points_[i].z());
        } else {
          ROS_INFO(
    ...
    if (cell_frontier_viewpoints.size() > 0) {
      ...
      for (int i = 0; i < ed_->points_.size(); ++i) {
        ...
        // ===== Lang bias：用当前机体 pos/yaw 对候选点方位取 hist 奖励 =====
        double h = 0.0;
        double cost_biased = cost;
        if (lang_enable_ && lang_beta_ > 1e-9) {
        ...
        if (lang_debug_) {
          ROS_INFO("[ExplorationManager][LangBias-B] i=%d cost=%.2lf h=%.2lf cost'=%.2lf",
                  i, cost, h, cost_biased);
        }
      ...
      if (cost > 499.0) {
        ...
        for (int i = 0; i < ed_->points_.size(); ++i) {
          ...
          vector<Eigen::Vector3d> path;
          double cost_i = PathCostEvaluator::computeCost(pos, ed_->points_[i], yaw[0], ed_->yaws_[i],
                                                        vel, yaw[1], path);
          ...
          if (lang_debug_) {
            ROS_INFO("[ExplorationManager][LangBias-C] i=%d cost=%.2lf h=%.2lf cost'=%.2lf",
                    i, cost_i, h, cost_biased);
          }
        }
      }
...
static inline double wrapToPi(double a) {
  ...
  void ExplorationManager::cueHistCb(const std_msgs::Float32MultiArrayConstPtr& msg) {
    cue_hist_ = msg->data;
    cue_hist_stamp_ = ros::Time::now();
